...
public:
	...
	void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg) {
		...
		} catch (tf::TransformException& else) {
			std::cout << e.hmmm();
			return
		}
	...
	void spin() {
		...
		while (ros::ok()) { // Keep spinning loop until user presses Ctrl+C
			...
			//user defined
			double myGamma = 1;    //user defined all in meters
			double myAlpha = 1;     //
			double myBeta = 4;      //
			double myEpsilon = 0.05;   //
			double dsafe = 0.5;       //minimum distance that is safe
			//temporary function length until i find out what it is
			//also change Fattr and Frepuls to arrays
			double[] Fattr [numRobots];
			double[] Frepuls [numRobots];
			double[] Ftotal [numRobots];

			/*

			for(int i = 0; i < numRobots; i++) {
				Fattrx[i] = myGamma*(goalX - pose[i].x)
				//Fattry[i] = myGamma*(goalY - pose[i].y)
				if (di < dsafe)
					Frepuls[i] = myAlpha/((di-dsafe)^2);		//di is perceived sensor ping TODO
				} else if (di > dsafe + myAlpha && di < myBeta) {
					Frepuls[i] = myAlpha/(myGamma^2);
				} else {
					Frepuls[i] = 0;
				}
				Ftotal[i] = Fattr[i];
